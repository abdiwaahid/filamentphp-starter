    ...
    public static function configure(Table $table): Table
    {
        return $table->modifyQueryUsing(fn($query) => $query->orderBy('created_at', 'desc'))
            ->columns([
                ...
                TextColumn::make('causer.name')->label(__('users::messages.activity.causer'))->sortable(),
                TextColumn::make('created_at')->label(__('users::messages.activity.logged_at'))->dateTime()->sortable(),
            ])
            ->filters([
                //
            ])
